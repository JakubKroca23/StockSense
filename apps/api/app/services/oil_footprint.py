"""WTI footprint from Bybit public trades — 1m bars persisted so history survives restarts."""
...
from app.services.oil_bybit import OIL_BYBIT_SYMBOL, _TICK, _WS_URL, fetch_clusdt_trades
...
# 1m bar_ts_ms -> price -> [buy, sell]
_levels: dict[int, dict[float, list[float]]] = {}
_ohlc: dict[int, list[float]] = {}
_dirty: set[int] = set()
_lock = asyncio.Lock()
_seen: set[str] = set()
_SEEN_CAP = 8_000
...
def _tick(price: float) -> float:
    return round(round(price / _TICK) * _TICK, 2)


def _ingest_unlocked(ts_ms: int, price: float, size: float, side: str, trade_id: str = "") -> None:
    if trade_id:
    ...
    levels = _levels.setdefault(bar_ts, {})
    cur = levels.get(p)
    if cur is None:
        levels[p] = [buy, sell]
    else:
        cur[0] += buy
        cur[1] += sell
    ohlc = _ohlc.get(bar_ts)
    if ohlc is None:
        _ohlc[bar_ts] = [p, p, p, p]
    else:
        ohlc[1] = max(ohlc[1], p)
        ohlc[2] = min(ohlc[2], p)
        ohlc[3] = p
...
async def ingest_trade(ts_ms: int, price: float, size: float, side: str, trade_id: str = "") -> None:
    async with _lock:
        _ingest_unlocked(ts_ms, price, size, side, trade_id)
...
def _pack_bar(ts_ms: int, levels: dict[float, list[float]], ohlc: list[float]) -> dict:
    rows: list[dict] = []
    poc = None
    poc_vol = -1.0
    buy_tot = 0.0
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            for ts_ms, lvls, ohlc in payload:
                packed = _pack_bar(ts_ms, lvls, ohlc)
                rows.append(
                    {
            ...
            if _last_prune is None or now - _last_prune >= PRUNE_EVERY:
        ...
        async with _lock:
            _dirty.update(ts for ts, _, _ in payload)
        raise
    async with _lock:
        _trim_memory()
    return len(payload)
...
async def snapshot_footprint(interval: str, lookback: str = "1d") -> dict:
    ...
    # Don't pull more 1m rows than we can reasonably serialize.
    ...
    store, meta = await _load_db(since, now)
    since_ms = int(since.timestamp() * 1000)
    async with _lock:
        for ts, lvls in _levels.items():
            if ts < since_ms:
                continue
            ...
            if ohlc:
                meta[ts] = list(ohlc)
    if iv != NATIVE:
        store, meta = _aggregate(store, meta, width)
    keys = sorted(store)
    if max_out > 0:
        keys = keys[-max_out:]
    bars = [_pack_bar(ts, store[ts], meta.get(ts) or [0, 0, 0, 0]) for ts in keys if store[ts]]
    last = bars[-1] if bars else None
    return {
        "symbol": "WTI",
        "source": "bybit:CLUSDT:trades",
        "interval": iv,
        "lookback": lb,
        "tick": _TICK,
        "bars_count": len(bars),
...
async def _backfill() -> None:
    try:
        payload = await fetch_clusdt_trades(limit=1000)
    except Exception:
        logger.exception("footprint backfill failed")
        return
    ...
    logger.info("oil footprint backfill %s trades", len(rows))
    try:
        n = await persist_dirty()
        if n:
            logger.info("oil footprint flushed %s bars after backfill", n)
    except Exception:
        logger.exception("oil footprint backfill persist failed")
...
async def _flush_loop(stop: asyncio.Event) -> None:
    while not stop.is_set():
        try:
            n = await persist_dirty()
            if n:
                logger.debug("oil footprint flushed %s bars", n)
        except Exception:
            logger.exception("oil footprint persist failed")
        try:
...
async def _ws_loop(stop: asyncio.Event) -> None:
    import websockets

    topic = f"publicTrade.{OIL_BYBIT_SYMBOL}"
    while not stop.is_set():
        try:
            logger.info("oil footprint ws → %s (%s)", _WS_URL, topic)
                ...
                async for raw in ws:
                    ...
                    async with _lock:
                        for row in rows:
                            ...
                            _ingest_unlocked(ts_ms, price, size, side, tid)
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("oil footprint ws dropped")
            try:
...
async def run_oil_footprint(stop: asyncio.Event) -> None:
    """Backfill recent prints, stream live trades, flush 1m bars to Postgres."""
    await asyncio.sleep(1.5)
    await _backfill()
    flusher = asyncio.create_task(_flush_loop(stop))
    try:
        await _ws_loop(stop)
    finally:
        flusher.cancel()
        ...
        try:
            ...
            logger.exception("oil footprint final flush failed")
