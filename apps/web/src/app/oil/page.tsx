...
function applyLiveBar(prev: OilChartResponse, live: ChartBar, extra?: Partial<OilChartResponse>): OilChartResponse {
...
export default function OilPage() {
  const [data, setData] = useState<OilChartResponse | null>(null);
  ...
  const [bookOpen, toggleBook] = usePersistedOpen("stocksense-oil-ob", true);
  const [tapeOpen, toggleTape] = usePersistedOpen("stocksense-oil-tape", true);
  ...
  const loadOrderBook = useCallback(
    async (forHeatmap: boolean) => {
      try {
        ...
        const res = await apiFetch<OrderBookData>(`/oil/orderbook?limit=${depth}`);
        setOrderBook(res);
  ...
  const loadTrades = useCallback(async () => {
    try {
      const res = await apiFetch<TradesTapeData>("/oil/trades?limit=90");
      setTradesTape(res);
    } catch {
      /* keep last tape */
    }
  ...
  const load = useCallback(async (iv: string, lb: string, silent = false) => {
    ...
    try {
      const res = await apiFetch<OilChartResponse>(
        `/oil/chart?interval=${encodeURIComponent(iv)}&lookback=${encodeURIComponent(lb)}`
      );
      setData(res);
      setError(null);
    } catch (err) {
      if (!silent) setError(err instanceof Error ? err.message : "Načtení WTI selhalo");
    } finally {
    ...
    const tick = async () => {
      try {
        const res = await apiFetch<FootprintData>(
          `/oil/footprint?interval=${encodeURIComponent(timeframe)}&lookback=${encodeURIComponent(lookback)}`
        );
  ...
  useEffect(() => {
    ...
    const connect = () => {
      if (closed) return;
      const url = apiWsUrl(`/oil/ws/ohlcv?interval=${encodeURIComponent(tf)}`);
    ...
    const tick = async () => {
      ...
      try {
        const snap = await apiFetch<OilLiveResponse>(
          `/oil/live?interval=${encodeURIComponent(timeframe)}`
        );
        if (cancelled) return;
      ...
      <HeaderExtra>
        <div className="header-desk">
          <div className="header-desk__id">
            <p className="header-desk__title">ROPA WTI</p>
            <p className="header-desk__quote">
              <span className="badge">{data?.symbol || "WTI"}</span>
              {live && (
                <span className="header-desk__live" title="Live Bybit CLUSDT">
        ...
        <div className="instrument-chart__stage crypto-chart-stage gold-page__chart-pane">
          {data?.bars?.length ? (
            footprint ? (
              <FootprintChart
                key={timeframe}
                data={fpData || { interval: timeframe, tick: 0.01, bars: [] }}
                viz={fpViz}
              />
            ) : (
              <PriceChart
            ...
            <div className="muted p-6 text-sm">
              {loading ? "Stahuji WTI…" : "Žádná OHLCV data."}
            </div>
          )}
