...
function fmtPrice(n: number | null | undefined, digits: number) {
...
export function DomPanel({
  ...
  const rowHeight = Math.max(12, Math.min(40, settings.rowHeight));
  const heatWidth = settings.showHeatmap ? Math.max(40, Math.min(240, settings.heatWidth)) : 0;
  const offset = settings.centerLock ? 0 : offsetRows;
  const rowCount = settings.centerLock ? fitRows : Math.max(6, settings.rows);
  ...
  }, [book, tape, footprint, tracker, settings, step, digits, offset, rowCount]);
  ...
  const onWheel = useCallback(
    (e: React.WheelEvent<HTMLDivElement>) => {
      ...
      setOffsetRows((v) => v + (e.deltaY > 0 ? -1 : 1));
    },
    [settings.centerLock]
  );
      ...
      extra={
        book ? (
          <p className="muted text-xs">
            spread {book.spread_pct != null ? `${book.spread_pct.toFixed(3)} %` : "—"} · krok{" "}
            {step.toFixed(digits)} · {Math.round(imbalance * 100)} % bid
          </p>
        ) : null
      }
    />
  ...
  return (
    <aside className="dom">
      ...
      <div
        ...
        style={{ gridTemplateColumns: gridTemplate, marginLeft: heatWidth }}
      >
      ...
      <div className="dom__grid" ref={gridRef} onWheel={onWheel}>
        ...
        <div className="dom__rows" style={{ marginLeft: heatWidth }}>
          {model.rows.map((row) => {
      ...
      <div className="dom__foot muted text-xs">
        <span>{book.execution_exchange}</span>
        {offset !== 0 ? (
        ...
        <button
          type="button"
          className={`dom__lock${settings.centerLock ? " is-active" : ""}`}
          onClick={() => {
            setOffsetRows(0);
            onSettingsChange({ centerLock: !settings.centerLock });
          }}
          aria-pressed={settings.centerLock}
        >
          {settings.centerLock ? "Zámek na mid" : "Volný sken"}
        </button>
      </div>
    </aside>
  );
